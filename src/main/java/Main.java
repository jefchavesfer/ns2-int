...
class Main {
	...
	public static void main(String args[]) throws IOException {
		...
		WirelessNodeSimulationProcessor.createSimulationFiles(wiredSimulationData, simulationParams.getWirelessFileDiscriminator());
		...
		while(!simulationParams.isConverged()){
			...
			WirelessFileProcess wirelessTrFile = new WirelessFileProcess(simulationParams.getWirelessFileDiscriminator(), numberOfScripts, simulationParams.getNumberOfClusters());
			...
			for(int i = 0; i < wiredSimulationData.size(); i++){
				...
				log.info("delivery Date -> wired: " + wiredData.getDeliveryRate() + "; wireless: " + wirelessData.getDeliveryRate());
				log.info("mean Delay    -> wired: " + wiredData.getMeanDelay() + "; wireless " + wirelessData.getMeanDelay());
				log.info("flow Data"  );
			...
			SimulationIterator simuIterator = new SimulationIterator(wiredSimulationData, wirelessSimulationData, simulationParams);
